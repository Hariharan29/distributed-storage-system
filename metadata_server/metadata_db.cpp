...
void MetadataDB::incrementAccessCount(const std::string& chunk_id) {
    std::lock_guard<std::mutex> lock(mutex_);
    sqlite3_stmt* stmt = nullptr;
    check(sqlite3_prepare_v2(db_, "UPDATE chunks SET access_count = access_count + 1 WHERE chunk_id = ?;", -1, &stmt, nullptr), db_, "prepare access increment");
...
void MetadataDB::updateNodeStatus(const std::string& id, bool alive) {
    std::lock_guard<std::mutex> lock(mutex_); sqlite3_stmt* stmt = nullptr;
...
std::vector<ChunkRecord> MetadataDB::getHotChunks(int threshold) {
    std::lock_guard<std::mutex> lock(mutex_); sqlite3_stmt* stmt = nullptr;
    check(sqlite3_prepare_v2(db_, "SELECT chunk_id, 0, sha256, size_bytes, access_count FROM chunks WHERE access_count >= ?;", -1, &stmt, nullptr), db_, "prepare hot chunks lookup");
    sqlite3_bind_int(stmt, 1, threshold); std::vector<ChunkRecord> chunks;
    while (sqlite3_step(stmt) == SQLITE_ROW) chunks.push_back(readChunk(stmt));
    sqlite3_finalize(stmt); return chunks;
}
